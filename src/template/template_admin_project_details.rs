...
async fn admin_project_details(
    ...
    let status = period_status.iter().filter(|s| s.project == project.id).collect::<Vec<_>>();
    let uptime_percent = ((status.iter().filter(|s| s.is_success()).count() as f32)/ (status.len() as f32)) * 100.0;
